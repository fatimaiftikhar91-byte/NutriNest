...
with st.sidebar:
    ...
    with st.expander("➕ Add / Update Member", expanded=False):
        with st.form("add_member"):
            ...
            medical = st.multiselect("Medical", ["none", "diabetes", "hypertension", "thyroid", "pcos"], default=["none"])
    ...
    st.session_state.budget["daily"] = st.number_input(
        "Daily Budget (PKR)", 300, 5000, st.session_state.budget["daily"], 50
    )

# ===================== HEADER =====================
st.markdown('<p class="big-title">🥗 NutriNest</p>', unsafe_allow_html=True)
st.markdown('<p class="subtitle">Smart Family Nutrition • Shared Meals • Personalized Portions</p>', unsafe_allow_html=True)
    ...
    if not st.session_state.family:
        ...
        for i, m in enumerate(st.session_state.family):
            with cols[i % len(cols)]:
                ...
                <div class="card">
                    <h3>{m['name']}</h3>
                    <p style="color:#57534E;">{m['goal'].replace('_',' ').title()} • BMI {n['BMI']}</p>
                    ...
                    <div>
                        ...
                        <span class="pill">Fiber ~{n['Fiber']}g</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with st.expander("Plan Preferences", expanded=True):
            ...
            with col2:
                prefs = st.multiselect(
                    "Meal Preferences",
                    ["Less oil", "High protein", "More fiber", "Vegetarian options", "Quick to cook"],
                    default=["Less oil", "High protein"]
                )

        if st.button("Generate Detailed Meal Plan", type="primary", use_container_width=True):
            with st.spinner("Creating detailed plate descriptions..."):
                plan = {
                    "day": "Day 1",
                    "meals": [
                        {
                            "meal": "Breakfast",
                            "main": "Vegetable Omelette + 1 Whole Wheat Roti",
                            "sides": ["Cucumber slices", "Green tea"],
                            "calories": 380,
                            "protein": 22,
                            "carbs": 28,
                            "fat": 18,
                            "fiber_tip": "Add 1 tbsp flax seeds or extra salad",
                            "protein_tip": "Eggs already provide high quality protein",
                            "portions": {m["name"]: "1 plate" for m in st.session_state.family},
                            "description": "Fluffy vegetable omelette cooked with minimal oil."
                        },
                        {
                            "meal": "Lunch",
                            ...
                            "carbs": 45,
                            "fat": 18,
                            "fiber_tip": "Brown rice + salad provide good fiber",
                            "protein_tip": "Chicken is an excellent lean protein source",
                            "portions": {m["name"]: "1 serving" for m in st.session_state.family},
                            "description": "Home-style chicken karahi made with less oil."
                        },
                        {
                            "meal": "Evening Snack",
                            "main": "Chana Chaat",
                            "sides": ["Lemon wedge", "Fresh coriander"],
                            "calories": 240,
                            "protein": 11,
                            "carbs": 38,
                            "fat": 5,
                            "fiber_tip": "Chickpeas are naturally high in fiber",
                            "protein_tip": "Add a boiled egg if needed",
                            "portions": {m["name"]: "1 bowl" for m in st.session_state.family},
                            "description": "Protein and fiber packed chickpea chaat."
                        },
                        {
                            "meal": "Dinner",
                            "main": "Dal Tadka + 2 Whole Wheat Rotis",
                            "sides": ["Cucumber raita"],
                            "calories": 480,
                            "protein": 22,
                            "carbs": 62,
                            "fat": 14,
                            "fiber_tip": "Lentils are excellent for fiber",
                            "protein_tip": "Add grilled chicken for higher protein",
                            "portions": {m["name"]: "1 serving" for m in st.session_state.family},
                            "description": "Comforting dal tadka with soft rotis."
                        }
                    ],
                    "notes": f"Focus on {', '.join(cuisines)}. Daily budget target ≈ Rs {st.session_state.budget['daily']}."
                }
        ...
        if st.session_state.meal_plan:
            ...
            for meal in plan.get("meals", []):
                ...
                <div class="meal-card">
                    ...
                    <p style="color:#44403C;">{meal.get('description', '')}</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                st.info(f"🌿 **Fiber tip:** {meal.get('fiber_tip', 'Add salad')}")
...
# ===================== PANTRY =====================
    ...
    st.caption("Add ingredients you already have. Meal plan will prefer these items.")
    new_item = st.text_input("Add item (e.g. chicken, daal, rice, yogurt, onion)")
    ...
    if st.session_state.pantry:
        ...
        st.info("Pantry is empty. Add common items like chicken, daal, rice, onion, tomato, yogurt, eggs.")
    ...
    if not st.session_state.family:
        ...
        if st.session_state.logs:
            for m in st.session_state.family:
                ...
                st.write(f"**{m['name']}**")
                st.write(f"Meals logged: {len(meals)} | Followed: {sum(1 for x in meals if x['status']=='followed')} | Workouts completed: {sum(1 for x in workouts if x['status']=='completed')} | Total minutes: {sum(x.get('minutes',0) for x in workouts)}")
        else:
            st.info("No logs yet.")
